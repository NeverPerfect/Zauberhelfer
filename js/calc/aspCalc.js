...
function berechneAsp() {
    ...
    // 2. Varianten: AsP-Werte aller ausgewählten Varianten summieren
    let variantenAsp = 0;
    ...
    // 3. Sonstige Modifikationen: AsP-Wert aus dem Feld holen
    let sonstigeAsp = 0;
    ...
    // 4. Grundkosten berechnen (inkl. Erzwingen, Varianten, Sonstige Modifikationen)
    let grundkostenGesamt = grundkosten + erzwingenWert + variantenAsp + sonstigeAsp;

    // 5. Kosten einsparen (10%, 20%, etc. aus Dropdown)
    ...
    if (kostenEinsparenChecked) {
        ...
        grundkostenGesamt = Math.max(1, Math.round(grundkostenGesamt * (1 - prozent / 100)));
    }
    ...
    let aspProEinheit = kosteneinheit * anzahlEinheiten;

    // 7. Gesamtkosten (Grundkosten + AsP/X)
    let aspGesamt = grundkostenGesamt + aspProEinheit;

    // 8. Sonderfertigkeiten: Kraftfokus und Kraftkontrolle (jeweils -1)
